...
- `crates/cores/n64` → `n64-core` (vendored Mupen64Plus-Next software core, privately loaded embedded library)
...
- **`revive-cli`** — The SDL3/wgpu/egui frontend. One ROM at a time, single event loop in `run_sdl_loop`. `FrameClock` paces the main thread using per-system native refresh rates (NES/SNES 60.0988, MD 59.9227, PCE 60, GB/GBA 59.7275; PS1 and N64 report the active NTSC/PAL rate). Each frame: poll events → apply cheats → `step_frame` → re-apply cheats → drain audio into an SDL3 audio stream → upload format-tagged frame data into a wgpu texture → render the optional egui cheat panel. Key mapping is per-system (`nes_keycode_button`, `snes_keycode_button`, etc.); state slots are handled in `handle_state_key` with Cmd+1-9 for load and Cmd+Shift+1-9 for save on macOS, and Ctrl+1-9 / Ctrl+Shift+1-9 on Windows/Linux.
...
- **Nintendo 64**: `N64Adapter` uses a thread-confined Mupen64Plus-Next cached interpreter, CXD4 RSP and Angrylion software renderer. `.z64`/`.n64`/`.v64` ROMs normalize by magic header; no ZIP support. 8 MiB Expansion Pak and one controller with Controller Pak are enabled. `set_stick` supplies SDL-style signed axes separately from D-pad/C buttons. RAM region `rdram` is mirrored in guest big-endian byte order; native word-swapped writes use offset XOR 3. The embedded private shared image isolates symbols from PS1. Saves are atomic in `states/n64/<stem>/cartridge.srm`; states use `.n64st` and a ROM-bound checksum envelope. Read `crates/cores/n64/UPSTREAM.md` for native patches, licensing and opt-in tests. `cargo test -p n64-core` includes an asset-free ARM64 coroutine ABI regression.

### State and save files

Save states live in `states/<system>/<rom-stem>/slot<N>.<ext>` (relative to CWD), created on demand. Extensions: NES `.sav`, SNES `.sns`, SG-1000 `.sgs`, Master System `.smsst`, MD `.mdst`, PCE `.pcst`, GBA `.gbas`, PS1 `.psst`, N64 `.n64st`. PS1 card 1 and N64 cartridge/Controller Pak data are also flushed every 60 frames when changed. Other SRAM/backup is separate and flushed on clean exit only (`core.flush_persistent_save()` in the shutdown path — crashes skip it). Cheats default to `cheats/<system>/<rom-stem>/cheats.json`.
