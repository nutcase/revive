...
impl Emulator {
    pub fn new(rom: Vec<u8>) -> Result<Self, String> {
        if rom.is_empty() {
            return Err("SG-1000 ROM is empty".to_string());
        }
        Ok(Self {
    ...
    pub fn load_state_bytes(&mut self, bytes: &[u8]) -> Result<(), String> {
        *self = emulator::load_state_bytes(&STATE_MAGIC, "SG-1000", bytes)?;
        Ok(())
    }

    pub fn load_state_from_file(&mut self, path: &Path) -> Result<(), String> {
        *self = emulator::load_state_from_file(&STATE_MAGIC, "SG-1000", path)?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn save_state_bytes_include_magic_header() {
        ...
        assert!(bytes.starts_with(b"SGS1"));
    }
