...
impl Cartridge {
    ...
    pub fn read_chr(&self, addr: u16) -> u8 {
        ...
        match self.chr_read_handler() {
            ...
            ChrReadHandler::Vrc7 => self.read_chr_vrc7(addr),
    ...
    fn chr_read_handler(&self) -> ChrReadHandler {
        match self.mapper {
            ...
            _ if self.uses_vrc6() => ChrReadHandler::Vrc6,
            _ if self.uses_vrc7() => ChrReadHandler::Vrc7,
            25 => ChrReadHandler::Mapper25,
            18 => ChrReadHandler::Mapper18,
            _ if self.uses_namco163() => ChrReadHandler::Namco163,
            _ if self.uses_mmc5() => ChrReadHandler::Mmc5,
            ...
            _ if self.uses_vrc1() => ChrReadHandler::Vrc1,
            13 => ChrReadHandler::Cprom,
            184 => ChrReadHandler::Split4k,
            _ if self.uses_taito_tc0190() => ChrReadHandler::TaitoTc0190,
            ...
            _ if self.uses_taito_x1005() => ChrReadHandler::TaitoX1005,
            _ if self.uses_taito_x1017() => ChrReadHandler::TaitoX1017,
            _ if self.uses_namco108() => ChrReadHandler::Namco108,
            _ if self.uses_mmc2_latches() => ChrReadHandler::Mmc2,
            _ if self.uses_bandai_fcg() => ChrReadHandler::Bandai,
            _ if self.uses_fme7() => ChrReadHandler::Fme7,
