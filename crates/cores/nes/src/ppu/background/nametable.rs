...
impl Ppu {
    ...
    pub(in crate::ppu) fn resolve_nametable(
        ...
        if let Some(cart) = cartridge {
            if cart.is_nrom() {
                return Self::resolve_mirrored_nametable(cart.mirroring(), logical_nt);
            }

            if let Some(table) = cart.resolve_nametable(logical_nt) {
                return table;
            }
            Self::resolve_mirrored_nametable(cart.mirroring(), logical_nt)
        } else {
            logical_nt & 1
        }
    }
    ...
    pub(in crate::ppu) fn read_nametable_byte(
        ...
        if let Some(cart) = cartridge {
            if cart.is_nrom() {
                return self.nametable[physical_nt & 1][offset];
            }
